...
def has_ruxcrypto_conf():
    import config
    import io

    valid_ruxcrypto_conf = False

    # ensure ruxcrypto_conf exists & readable
    #
    # if not, print a message stating that RXC Core must be installed and
    # configured, including JSONRPC access in ruxcrypto.conf
    try:
        f = io.open(config.ruxcrypto_conf)
        valid_ruxcrypto_conf = True
    except IOError as e:
        print(e)

    return valid_ruxcrypto_conf
...
def main():
    ...
    from sib_config import RXCConfig
    config.sentinel_cfg = RXCConfig.tokenize(config.sentinel_config_file)

    config.ruxcrypto_conf = config.get_ruxcrypto_conf()
    ...
    if not has_ruxcrypto_conf():
        print("DashCore must be installed and configured, including JSONRPC access in ruxcrypto.conf")
        sys.exit(1)
