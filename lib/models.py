...
class GovernanceObject(BaseModel):
    ...
    def vote(self, ruxcryptod, signal, outcome):
        ...
        voted = ruxcoinlib.did_we_vote(output)
...
class Proposal(GovernanceClass, BaseModel):
    ...
    def is_valid(self):
        ...
        try:
            ...
            if not ruxcoinlib.is_valid_address(self.payment_address, config.network):
    ...
    def is_expired(self, superblockcycle=None):
        ...
        expiration_window_seconds = int(
            (ruxcoinlib.blocks_to_seconds(superblockcycle) / 2) +
...
class Superblock(BaseModel, GovernanceClass):
    ...
    def is_valid(self):
        ...
        for addr in addresses:
            if not ruxcoinlib.is_valid_address(addr, config.network):
    ...
    def hash(self):
        import ruxcoinlib
        return ruxcoinlib.hashit(self.serialise())
