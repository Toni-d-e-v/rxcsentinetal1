...
class RXCConfig(DashConfig):

    @classmethod
    def get_rpc_creds(self, data, network='mainnet'):
        # get rpc info from ruxcrypto.conf
        match = re.findall(r'rpc(user|password|port)=(.*?)$', data, re.MULTILINE)

        # python >= 2.7
        creds = {key: value for (key, value) in match}

        # standard RXC defaults...
        default_port = 33505 if (network == 'mainnet') else 23505

        # use default port for network if not specified in ruxcrypto.conf
