...
def valid_dash_address(network='mainnet'):
    return 'RSeiKWBuzm2j3oVHCnTtFZFGFVKaJQacpt' if (network == 'testnet') else 'RSeiKWBuzm2j3oVHCnTtFZFGFVKaJQacpt'


@pytest.fixture
def invalid_dash_address(network='mainnet'):
    return 'RBKgf7Vonrcb7aQKKi7rZtJkcu4qn5cihV' if (network == 'testnet') else 'RHipXvksQNFBATjwXWUZawvNHS8W8MNAhV'
...
def test_invalid_dash_address():
    ...
    assert is_valid__address(main, 'testnet') is False
